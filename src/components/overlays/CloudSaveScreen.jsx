...
export default function CloudSaveScreen({ onClose }) {
  ...
  const sync = async () => { setMsg('同步中……'); const r = await uploadCloudSave(); setMsg(r.ok ? '云存档已上传。' : '同步失败：' + (r.error?.message || '离线/未配置')); };
  const saveName = async () => { const r = await updateNickname(name); setMsg(r.ok ? '昵称已更新。' : '昵称更新失败。'); };
  const reconnect = async () => { setMsg('连接中……'); await initOnline(); setMsg(getOnlineState().online ? '已连接服务器。' : '仍为离线：' + getOnlineState().lastError); };
  return <div className="modal-back"><div className="modal paper small-modal">
    ...
    <div className="quote">
      ...
      {online.lastError && <>提示：{online.lastError}</>}
    </div>
    ...
    <div className="tip-line">没网也能玩；结算成绩会进入待上传队列。</div>
  </div></div>;
}
